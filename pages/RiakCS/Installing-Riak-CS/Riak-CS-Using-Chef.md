...
<div class="note">This automatic configuration recipe works only with a full Chef Stack, not chef‐solo.</div>
...
<div class="note">Running Stanchion on more than one node can lead to problems if Riak CS nodes are configured to communicate using multiple Stanchion nodes. In this situation, the uniqueness of bucket names and user email addresses might not be enforced, which, in turn, could lead to unexpected behavior.</div>
