...
This script is used to interact with the Search functionality included with Riak.  Make sure you've first "enabled Search":http://wiki.basho.com/Riak-Search---Installation-and-Setup.html.  The @INDEX@ argument is optional for all commands and defaults to @test@.
...
Set the "schema":http://wiki.basho.com/Riak-Search---Schema.html for a given index.  If you don't explicity set the schema for an index it will use the default schema.
...
Show the "schema":http://wiki.basho.com/Riak-Search---Schema.html for a given index.
...
Execute the given query on the index returning the document id, properties, and score.  The "query syntax":http://wiki.basho.com/Riak-Search---Querying.html is the same as Lucene.
...
bc. explain [INDEX] QUERY

TODO - should we really be exposing this in search-cmd?  It exposes internal structure of parsed queries.  That is useful to us, internally, but I'm not sure it's something we want users to become attached to as these things are implementation details.
...
Index the document at the given path.  See the "indexing section":http://wiki.basho.com/Riak-Search---Indexing.html#Indexing-from-the-Command-Line for more details.
...
Used to delete a document from the index.  See the "indexing section":http://wiki.basho.com/Riak-Search---Indexing.html#Deleting-from-the-Command-Line for more details.
...
Index solr documents.  See the "indexing section":http://wiki.basho.com/Riak-Search---Indexing.html#Indexing-using-the-Solr-Interface for more details.
...
Install the Search precommit hook on the given bucket.  This allows one to "index incoming objects":http://wiki.basho.com/Riak-Search---Indexing-and-Querying-Riak-KV-Data.html.
...
TODO - Create a page about writing test scripts?