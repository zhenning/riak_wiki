...
If you have upgraded to OS/X Lion, using @/etc/launchd.conf@ to set maxfiles will actually reduce the number of files a process can have open from the default of 10240. The most effective approach we have found for local development on OS/X is to remove the limit on maxfiles from @/etc/launchd.conf@ and add @ulimit -n 4096@ to your @~/.profile@ or equivalent.
