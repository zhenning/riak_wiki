...
* @pr@ - how many primary replicas need to be online when doing the read ([[default is defined by the bucket|HTTP API Set Bucket Properties]])
* @basic_quorum@ - whether to return early in some failure cases (eg. when r=1 and you get 2 errors and a success basic_quorum=true would return an error) ([[default is defined by the bucket|HTTP API Set Bucket Properties]])
* @notfound_ok@ - whether to treat notfounds as successful reads for the purposes of R ([[default is defined by the bucket|HTTP API Set Bucket Properties]])
