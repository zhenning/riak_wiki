For this tutorial, we are going to work with the HTTP Interface and use curl commands through the terminal to test Riak out and help you get acquainted with it quickly. You can also interact with Riak using the Protocol Buffers (PB) interface or the native Erlang interface, but we'll cover the HTTP REST API here. 
