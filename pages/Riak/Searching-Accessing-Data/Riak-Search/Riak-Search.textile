...
Riak Search allows you to find and retrieve your Riak objects using the objects' values. When a Riak KV bucket has been enabled for Search integration (by installing the Search pre-commit hook), Riak Search automatically indexes KV objects as they're written.  Before the object is inserted, the pre-commit hook extracts and analyzes the data being written and builds indexes from it (based on the object's mime type and the search schema you've set for its bucket).

The Riak Client API is used to invoke Search queries that return a list of bucket/key pairs matching the query. Currently the PHP, Python, Ruby, and Erlang APIs support integration with Riak Search.  Alternatively, the query results can be used as the input to a Riak map/reduce operation. 
