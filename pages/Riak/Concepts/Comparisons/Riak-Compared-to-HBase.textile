...
* Riak is written primarily in Erlang with some bits in C; HBase is written in Java

h2. Feature/Capability Comparison

The table below gives a high level comparison of Riak and HBase features/capabilities.  To keep this page relevant in the face of rapid development on both sides, low level details are found in links to Riak and HBase online documentation.
...
<table>
    ...
    <tr>
        ...
        <td> 
        	HBase's storage system is the Hadoop Distributed File System (HDFS). Data is stored in MemStores, and StoreFiles where data is streamed to disk (implemented via HFiles, a format based on BigTable's SSTable). Implementations generally use the native JVM-managed I/O file stream.
			<ul>
	...
	<tr>
        ...
        <td>HBase guarentees write atomicity and locks per row, but has no multi-operation (including multi-row) transactions. Scans have no consistency guarantees.
			<ul>
    ...
    <tr>
        ...
        <td>HBase supports optional replication by a standard eventually consistent master/slave push. Recently, HBase added (experimental, nor well documented) support for master/master and cyclic (where each node plays the role of master and slave) replication.
	...
	<ul>
        ...
        <td>HBase shards by way or regions, that automatically split and redistribute growing data. A crash on a region requires crash recovery. HBase does not excel in scaling in, however, requiring some manual intervention.
