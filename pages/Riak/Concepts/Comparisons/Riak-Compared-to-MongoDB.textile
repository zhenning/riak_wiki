...
<table>
    ...
    <tr>
        ...
        <td>Mongo’s data format is BSON (binary equivalent to JSON). Mongo stores this data as documents (self-contained records with no intrinsic relationships). Documents in MongoDB may store any of the defined BSON types, including raw binary data (upon which the GridFS feature is built).

			<ul>
			    
				<li>[[Data Types and Conventions|http://www.mongodb.org/display/DOCS/Data+Types+and+Conventions]]</li>
			  <li>[[BSON|http://www.mongodb.org/display/DOCS/BSON]]</li>
			</ul>
		</td>
    </tr>
    <tr>
        <td>Storage Model</td>
        <td>Riak has a modular, extensible local storage system which lets you plug-in a backend store of you choice to suit your use case. The default backend is Bitcask.  
			<ul>
        ...
        <td> MongoDB's default storage system is the Memory-Mapped Storage Engine.  It uses memory mapped files for all disk I/O.  It is the responsibility of the OS to manage flushing data to disk and paging data in and out.  An alternative storage engine was introduced in MongoDB 1.9+ called NuMap, which aims to increase performance for high-load applications.

			<ul>
			 <li>[[Cashing|http://www.mongodb.org/display/DOCS/Caching]]</li>
			<li>[[NuMap|http://numap-mongo.blogspot.com/]]</li>
			</ul>
		</td>
    </tr>
    <tr>
        ...
        <td>Riak offers two primary interfaces (in addition to raw Erlang access):
			...
			<ul>
			  ...
			  <li>[[Community Developed Clients and Projects]] </li>
			</ul>
			</td>
        <td> MongoDB uses a custom protocol with BSON as the interchange format, and 10gen supports clients in the most popular programming languages.
			<ul>
			  <li>[[Client-Libraries|http://www.mongodb.org/display/DOCS/Drivers]]</li>
			<li>[[API documentation|http://www.mongodb.org/display/DOCS/Online+API+Documentation]]</li>
			</ul>		
	 </td>
    </tr>
    <tr>
        ...
        <td>MongoDB has a query interface that has some similarities to relational databases, including secondary indexes that can be derived from the stored documents. MongoDB also has a facility for performing MapReduce queries.
				<ul>
	...
	<tr>
        ...
        <td> Riak uses a data structure called a vector clock to reason about causality and staleness of stored values. Vector clocks enable clients to always write to the database in exchange for consistency conflicts being resolved at read time by either application or client code. Vector clocks can be configured to store copies of a given datum based on size and age of said datum.   There is also an option to disable vector clocks and fall back to simple time-stamp based "last-write-wins". 	
			<ul>
			  <li>[[Vector Clocks]]</li>
			  <li>[[Why Vector Clocks Are Easy|]]</li>
			  <li>[[Why Vector Clocks Are Hard|]]</li>
			</ul>
		 </td>
    ...
    <tr>
        ...
        <td>Riak's replication system is heavily influence by the Dynamo Paper and Dr. Eric Brewer's CAP Theorem. Riak uses consistent hashing to replicate and distribute N copies of each value around a Riak cluster composed of any number of physical machines. Under the hood, Riak uses virtual nodes to handle the distribution and dynamic rebalancing of data, thus decoupling the data distribution from physical assets.
			<ul>
			  <li>[[Replication]]</li>
			  <li>[[Clustering|Concepts.html#Clustering]]</li>
			</ul>
			...
			The Riak APIs expose tunable consistency and availability parameters that let you select which level configuration is best for you use case. Replication is configurable at the bucket level when first storing data in Riak. Subsequent reads and writes to that data can have request-level parameters.
				<ul>
					<li>[[Reading, Writing, and Updating Data|Concepts.html#Reading%2C-Writing%2C-and-Updating-Data]]</li>
				</ul>	
			
	 </td>
        <td>Mongo manages replication via replica sets, a form of asynchronous [[master/slave replication|http://www.mongodb.org/display/DOCS/Master+Slave]].  Traditional master/slave replication is available but not recommended.
			<ul>	
	...
	<ul>
        ...
        <td>To enable horizontal scaling, Mongo uses a process known as [[sharding|http://en.wikipedia.org/wiki/Sharding]], which involves designating a certain server to hold certain chunks of the data as the data set grows.  

New in the 1.6 release is “auto-sharding.”  Autosharding allows the database to scale linearly by adding new machines.
    ...
    <tr>
        ...
        <td>MongoDB accomplishes multi-data center replication via [[replica sets|http://www.mongodb.org/display/DOCS/Replica+Sets]].  
	
			<ul>
			  <li>[[Data Center Awareness|http://www.mongodb.org/display/DOCS/Data+Center+Awareness]]</li>
			<li>[[Strategies for Multiple Data Center Deployment|http://www.slideshare.net/spf13/multi-data-center-awareness?ref=http://architects.dzone.com/articles/mongodb-strategies-multiple]]</li>
			<li>[[Deploy a Geographically Distributed Replica Set|http://docs.mongodb.org/manual/tutorial/deploy-geographically-distributed-replica-set/?highlight=data%20center]]</li>		
			</ul>
