...
* Riak and CouchDB are bot Apache 2.0 licensed
...
<table>
    ...
    <tr>
        ...
        <td>CouchDB's data format is JSON stored as documents (self-contained records with no intrinsic relationships).
			<ul>
    ...
    <tr>
        ...
        <td>CouchDB is queried by direct ID lookups, or by creating map reduce "views" that CouchDB runs to create a queryable index for querying by or computing other attributes. CouchDB allows temporary ad-hoc views.
	...
	<tr>
        ...
        <td>CouchDB replicates newer document versions between nodes, making it an eventually consistent system. Older document versions (called revisions) may be lost when the append-only database file is compacted.
			<ul>
        ...
        <td>Because of CouchDB's append-only value mutation, individual instances will not lock. When distributed, couch won't allow updating similarly keyed document without a preceeding version number, and conflicts must be manually resolved before concluding a write.
	...
	<ul>
        ...
        <td>CouchDB is focused largely on replication of values, which is a way to grow reads. For write replication, there are external projects that manage a cluter, such as BigCouch (also Apache 2.0 licensed), that shards values across multiple nodes.
