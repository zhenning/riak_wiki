...
Riak uses a [[backend API]] to interact with its storage subsystem. The API allows Riak to support multiple backends which can be selected based on use-cases. See the [[Storage Backend]] page for full list of what we currently support. The two most heavily-used backend are Bitcask and LevelDB.
