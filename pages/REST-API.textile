...
* *r, w, dw, rw, pr, pw* - default quorum values for operations on keys in the bucket. Valid values are:
...
* *pr* - how many primary replicas need to be online when doing the read ([[default is defined by the bucket|REST API#Set bucket properties]])
* *basic_quorum* - whether to return early in some failure cases (eg. when r=1 and you get 2 errors and a success basic_quorum=true would return an error) ([[default is defined by the bucket|REST API#Set bucket properties]])
* *notfound_ok* - whether to treat notfounds as successful reads for the purposes of R ([[default is defined by the bucket|REST API#Set bucket properties]])
